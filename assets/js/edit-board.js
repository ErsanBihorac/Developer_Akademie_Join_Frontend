...
async function acceptEdit(i) {
    const updatedData = EditValue(i);
...
function EditValue(i) {
    ...
    let description = document.getElementById(`edit-description${i}`);
    ...
    return task = {
        ...
        'assigned_to': todos[i].assignedName,
    };
}
...
function loadAssignableNamesEdit(id) { // this function loads the assignable contacts
    ...
    for (let i = 0; i < contacts.length; i++) {
        newID = 1 + id.toString() + i.toString();
        const initial = contacts[i]['first_letter'];
...
async function manipulateAssignedInitialsEdit(i, id, li) {
    const toBeAssigned = contacts[i]['firstetter'];
