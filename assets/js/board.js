...
function taskStateSwitchTo(i) {
  return task = {
    'title': todos[i].title,
    'description': todos[i].description,
    'due_date': todos[i].dueDate,
    'priority': todos[i].priority,
    'status': getState(todos[i].state),
    'assigned_to': [1, 2],
  };
}

async function updateTaskWithNewState(i) {
  const updatedData = taskStateSwitchTo(i);
  const csrfToken = localStorage.getItem('csrfToken');

  await fetch(`http://localhost:8000/api/tasks/${todos[i].id}/`, {
    method: 'PUT',
    headers: {
    ...
    <div class="cardFrame">
      ...
      <div id="card-subtask-${task.id}" class="cardProgress ${determineIfSubtaskExists(task)}">
        <div class="cardProgressbar">
          <div class="progress">
           <div class="progress-bar" role="progressbar" style="width: ${(taskSum(task) / 1) * 100
      }%; 
        ...
        task
         ...
         <div class="cardProgressText">${taskSum(task)}/1 Subtasks</div>
      </div>
      <div class="cardContacts">
        <div class="cardContactsBadge">
          <div class="cardAssignedInitials" id="cardAssignedNameContainer${task.id
...
function generatePlaceholder() {
  ...
  return `<div class="cardplaceholder">
    <div class="cardplaceholderLable"></div>
    <div class="cardTextI"></div>
    <div class="cardTextII"></div>
    <div class="cardTextII cardTextR"></div>
  </div>`;
}
/**
...
async function moveTo(state) {
  ...
  await updateTaskWithNewState(id);
