...
async function loadTasks() {
  ...
  if (response.ok) {
    await formatTodos(response);
  } else {
    console.error('Error creating todos:', response.statusText);
  }

  updateHTML();
}

async function formatTodos(response) {
  todos = await response.json();

  const formattedTasks = todos.map(task => {
    const assignedNames = task.assigned_to.map(user => user.username);
    const assignedInitials = task.assigned_to.map(user => {
    ...
    return formattedTodo(task, assignedNames, assignedInitials);
  });

  todos = formattedTasks
}

function formattedTodo(task, assignedNames, assignedInitials) {
  return {
    assignedInitial: assignedInitials,
    assignedName: assignedNames,
    description: task.description,
    dueDate: task.due_date,
    id: task.id,
    priority: task.priority,
    priorityImageSource: `../img/${task.priority}_no_bg.svg`,
    state: convertState(task.status),
    title: task.title,
  };
}
...
function renderTask(todo, id) {
  ...
  return `
      ...
      <div class="bOverlayText">description: ${todo.description}</div>
